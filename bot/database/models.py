...
class Client(Base):
    ...
    От формы регистрации на сайте в карточке остаются только номер телефона как
    ключ сверки и флаг согласия: карточка заводится не загрузчиком событий, а
    сверкой номера, который уже есть в базе. ФИО, город, пол, e-mail, дата
    рождения и телефон пригласившего из карточки убраны — на первичный диалог и
    на сегмент они не влияют.
    ...
    # --- данные формы регистрации сайта: телефон и согласие ---
    ...
    # Статус после того, как наставник нажал «связаться»: пустая строка — связи ещё не было.
    contact_offered_at: Mapped[str] = mapped_column(String(40), nullable=True)
    contact_offered_by: Mapped[str] = mapped_column(String(150), nullable=True)
